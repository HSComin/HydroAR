...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
    ...
    {
        [SerializeField]
        [Tooltip("Referência ao controlador de instanciação para verificar se deve bloquear instanciação manual.")]
        MonoBehaviour m_InstanciationController;

        /// <summary>
        /// Referência ao controlador de instanciação para verificar se deve bloquear instanciação manual.
        /// </summary>
        public MonoBehaviour instanciationController
        {
            get => m_InstanciationController;
            set => m_InstanciationController = value;
        }
        ...
        GameObject m_LastSpawnedObject = null;

        public bool TrySpawnObject(Vector3 spawnPoint, Vector3 spawnNormal)
        {
            var menuManagerType = System.Type.GetType("ARTemplateMenuManager");
            if (menuManagerType != null)
            {
                var instanciacaoField = menuManagerType.GetField("InstanciacaoBloqueada",
                    System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static);

                if (instanciacaoField != null)
                {
                    bool bloqueada = (bool)instanciacaoField.GetValue(null);
                    if (bloqueada)
                    {
                        Debug.Log("ObjectSpawner: Instanciação bloqueada - Tela Mapa ativa");
                        return false;
                    }
                }
            }
